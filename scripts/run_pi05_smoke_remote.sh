...
RESULT_DIR="$VLA_PROJECT/results/training/pi05_smoke_test"
OUTPUT_DIR="$RESULT_DIR/run"
MODEL_DIR=/root/autodl-tmp/cache/huggingface/pi05_libero_base
DATASET_DIR=/root/autodl-tmp/cache/huggingface/lerobot/lerobot/libero
...
if [[ -e "$OUTPUT_DIR" || -e "$RESULT_DIR/torch_step_metrics.jsonl" ]]; then
  echo "Refusing to overwrite an existing smoke-test run." >&2
  exit 2
fi
...
/root/miniconda3/bin/python \
  ...
  --job_name=pi05_libero_4090_smoke \
  --batch_size=1 \
  --num_workers=1 \
  --steps=50 \
  --log_freq=1 \
  ...
  --wandb.enable=false \
  --seed=1000 \
  2>&1 | tee "$RESULT_DIR/stdout_stderr.log"
