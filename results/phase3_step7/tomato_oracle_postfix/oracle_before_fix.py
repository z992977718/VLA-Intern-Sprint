...
def main() -> int:
    ...
    try:
        ...
        safety = {"pass": True, "events": [], "max_joint_step_rad": MAX_JOINT_STEP_RAD, "joint_limits_checked": True}
        ...
        def execute_stage(name: str, target_tool_position: np.ndarray, target_tool_rotation: np.ndarray, steps: int, finger_target: float) -> None:
            ...
            for index in range(steps):
                ...
                step_size = float(np.max(np.abs(joint_target - current[indices])))
                if not np.isfinite(joint_target).all() or step_size > MAX_JOINT_STEP_RAD or np.any(joint_target < lower[indices]) or np.any(joint_target > upper[indices]):
                    safety["pass"] = False; safety["events"].append({"stage": name, "step": index, "joint_step_rad": step_size})
